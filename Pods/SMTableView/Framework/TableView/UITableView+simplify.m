...
- (SMTableViewSimplifyModel *)simplifyModel {
    ...
    if (model == nil) {
        ...
        @synchronized (self) {
            model =  objc_getAssociatedObject(self, tableViewKeyForModel);
            if(model == nil){
                model = [[SMTableViewSimplifyModel alloc] init];
                objc_setAssociatedObject(self, tableViewKeyForModel, model, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
            }
        }
    }
    return model;
...
- (BOOL)clearsSelectionDelay {
    return objc_getAssociatedObject(self, tableViewKeyForClearsSelectionDelay);
}
//是否分块
...
- (BOOL)sectionable {
    return objc_getAssociatedObject(self, tableViewKeyForSectionable);
}
...
- (void)hookDataSource:(id)dataSource {
    ...
    @synchronized (self) {
        ...
        Class dClass = [self.simplifyModel class];
